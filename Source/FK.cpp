    ...
    vector<RigidTransform4d> & localTransforms, vector<RigidTransform4d> & globalTransforms)
{
  ...
  {
    // i only represent updating order
    // Get actual joint index
    int jointIndex = jointUpdateOrder[i];

    double rotationMatrix[9];
    double orientRotationMatrix[9];
    // local transform
    euler2Rotation(eulerAngles[jointIndex], rotationMatrix, rotateOrders[jointIndex]);
    euler2Rotation(jointOrientationEulerAngles[jointIndex], orientRotationMatrix, getDefaultRotateOrder());

    Mat3d postOrientLocalTran = Mat3d(orientRotationMatrix) * Mat3d(rotationMatrix);
    localTransforms[jointIndex] = RigidTransform4d(postOrientLocalTran, translations[jointIndex]);

    // global transform
    // special case for parent
    if (jointParents[jointIndex] == -1) 
    {
      globalTransforms[jointIndex] = localTransforms[jointIndex];
    }
    else
    {
      globalTransforms[jointIndex] = globalTransforms[jointParents[jointIndex]] * localTransforms[jointIndex];
    }
  }
}
